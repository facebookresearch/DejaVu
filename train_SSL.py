...
#            model.fc = nn.Linear(model.num_features, self.num_classes)
    ...
    @param('training.num_small_crops')
    def train_loop(self, epoch, log_level, loss, base_lr, end_lr_ratio, num_small_crops):
        """
    ...
    def initialize_logger(self, folder):
        ...
        for l in range(self.n_layers_proj):
            ...
            self.train_meters['top_1_layer'+str(l)] = torchmetrics.Accuracy('multiclass',num_classes=1000,top_k=1,compute_on_step=False).to(self.gpu)
            self.train_meters['top_5_layer'+str(l)] = torchmetrics.Accuracy('multiclass',num_classes=1000,top_k=5,compute_on_step=False).to(self.gpu)

        self.val_meters = {}
        for l in range(self.n_layers_proj):
            self.val_meters['loss_classif_val_layer'+str(l)] = torchmetrics.MeanMetric(compute_on_step=False).to(self.gpu)
            self.val_meters['top_1_val_layer'+str(l)] = torchmetrics.Accuracy('multiclass',num_classes=1000,top_k=1,compute_on_step=False).to(self.gpu)
            self.val_meters['top_5_val_layer'+str(l)] = torchmetrics.Accuracy('multiclass',num_classes=1000,top_k=5,compute_on_step=False).to(self.gpu)
