...
def interpolate_points(p1, p2, n_steps=1):
	# interpolate ratios between the points
	ratios = np.linspace(0.7, 0.7, num=n_steps)
...
def main(args):
    ...
    while num_current_samples < args.num_images:
        ...
        with th.no_grad():
            ...
            for k, sigma in enumerate([0, 0.05, 0.1, 0.15, 0.2, 0.25, 0.3, 0.35]):
                feat[k] += th.zeros_like(feat[k]).normal_(0, sigma)
            model_kwargs["feat"] = feat
            sample = sample_fn(
                model,
            ...
            batch = th.from_numpy(dataset.__getitem__(new_index[num_current_samples+1])[1])
            batch = batch.unsqueeze(0).repeat(args.batch_size, 1, 1, 1).cuda()
            feat_2 = ssl_model(batch).detach()
            #feat[:, lf[(c > 63)]] = 0
            #feat = sort_list_backbone[num_current_samples:num_current_samples+1].cuda().float()
            for k, sigma in enumerate([0, 0.05, 0.1, 0.15, 0.2, 0.25, 0.3, 0.35]):
                feat_2[k] += th.zeros_like(feat_2[k]).normal_(0, sigma)
            model_kwargs["feat"] = feat_2
            sample = sample_fn(
                model,
                (args.batch_size, 3, args.image_size, args.image_size),
                clip_denoised=args.clip_denoised,
            ...
            # model_kwargs["feat"] = interpolate_points(feat, feat_2)
            model_kwargs["feat"] = feat * 0.5 + 0.5 * feat_2
            # model_kwargs["feat"] = th.FloatTensor(interpolate_points(feat.cpu().numpy(), feat_2.cpu().numpy(), n_steps=args.batch_size)).cuda()
            print(model_kwargs["feat"].size())
