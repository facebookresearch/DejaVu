...
def get_attack_data(model, datasets, epochs, k_neighb, pct_confidences, topks, attk_folder_path): 
    #get attack data 
    base_dir = Path(attk_folder_path)
...
def get_linprobe_plots_epochs(model, dataset, epochs, pth, lin_probe_folder_path): 
    #get attack data 
    base_dir = Path(lin_probe_folder_path)
    ds = dataset
...
def get_linprobe_plots_datasets(model, datasets, epoch, pth, lin_probe_folder_path):
    #get saved data 
    base_dir = Path(lin_probe_folder_path)# Get linear probe results
    traces = defaultdict(list)
    ep = epoch
    for ds in datasets: 
        folder = base_dir / f'lp_{model}_{ds}pc_{ep}ep' 
...
def get_attack_data_v_params(model, param_vals, epochs, k_neighb, pct_confidences, topks, attk_folder_path): 
    #get attack data 
    base_dir = Path(attk_folder_path)
    traces = defaultdict(list)
...
def get_linprobe_plots_params(model, param_vals, epoch, pth, lin_probe_folder_path):
    if model == 'simclr':
        xlabel = 'Temperature, $\\tau$'
    else: 
        xlabel = 'Invariance, $\lambda$'
